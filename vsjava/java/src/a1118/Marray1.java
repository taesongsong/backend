...
public class Marray1 {
    public static void main(String[] args) {
        // 각반의 학생수가 3명으로 동일할 경우 점수 저장을 위한 2차원 배열 생석
    ...
    int totalSutudent =0;
    for(int i=0; i < mathScores.length; i++){ // 반의 수만큼 반복
        totalSutudent = totalSutudent + mathScores[i].length;
        for(int k = 0; k < mathScores[i].length; k++){ // 해당 반의 학생수 만큼 반복
         totalMathSum = totalMathSum + mathScores[i][k];
        }
      }
      System.out.println("전체 학생의 수학합계 : " + totalMathSum );
      System.out.println("전체 학생의 수 : " +totalSutudent);
      double totalMathAvg = (double)totalMathSum / totalSutudent;
